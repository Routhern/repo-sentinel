...
def track(path: Path = typer.Argument(..., help="추적할 git 저장소 경로")) -> None:
    """PATH를 구독 목록에 등록한다. 이때부터 pick/relink/audit 대상이 된다."""
...
def list_repos() -> None:
    ...
    subs = subscriptions.load_subscriptions()
    if not subs:
        console.print("구독 중인 저장소가 없습니다. 먼저 `repo-sentinel track`을 실행하세요.")
        raise typer.Exit()
...
def pick(
    ...
    sub = subs.get(repo_key)
    if sub is None:
        console.print(f"[red]{repo_key} 구독을 찾을 수 없습니다. 먼저 track하세요.[/red]")
        raise typer.Exit(code=1)

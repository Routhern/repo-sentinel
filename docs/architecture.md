...
5. `gitignore` (`core/gitignore.py`) — `pick`이 만드는 심볼릭 링크는 vault의
   ...
   사용자 확인을 받은 뒤에만 호출한다.
   - `ensure_region`/`read_region_patterns`/`resolve_patterns` — `track_repo`가
     레포의 `.gitignore`에 `REGION_BEGIN`/`REGION_END` 마커로 감싼
     "repo-sentinel 리전"을 관리한다. 리전이 없으면 전역 `sensitive_patterns`로
     새로 만들고, **이미 있으면 절대 덮어쓰지 않는다** — 패턴 추가/삭제는
     사용자가 그 구간을 직접 편집하는 수동 과정이다. `pick --auto`와
     `find_candidates`는 레포에 리전이 있으면 전역 설정 대신 이 리전의
     패턴을 우선 사용한다(`resolve_patterns`).
6. `protect`/`restore`/`relink` (`core/protect.py`) — 기준 상태는 vault다.
   ...
   - `reflect_gitignore`: `protect_file` 이후 `.gitignore` 반영 상태를
     정리하는 헬퍼. 이미 무시되어 있으면 매니페스트에 확인 표시만 하고,
     아니면 `should_add`일 때만 `gitignore.add_entry`를 호출한다 — 추가
     여부를 사용자에게 물을지는 호출자(CLI의 `typer.confirm`, TUI는 항상
     자동 추가)의 몫이다. CLI의 `pick`/`track`과 TUI의 `PickScreen`/
     `TrackScreen`이 모두 이 함수를 공유한다.
...
- `tui/screens.py` — 모든 화면(Screen)이 정의된 곳.
  ...
  - `TrackScreen`/`UntrackScreen`/`RelinkScreen` — 각각
    ...
    없는 작업이기 때문이다. `TrackScreen`은 등록 후 `track_repo`가 돌려준
    `pick_candidates`가 있으면 `ConfirmModal`을 하나씩 순서대로 띄워(콜백
    체이닝으로 다음 후보를 잇는다) 지금 pick할지 물어본다.
...
`ConfirmModal`)의 표현 계층 책임으로 남아 있다. `track_repo`는 등록에 더해
`.gitignore` 리전 생성(`gitignore.ensure_region`)과 그 리전 패턴에 매칭되는
pick 후보 계산(`protect.find_candidates`)까지 한 번에 처리하고
`TrackResult.gitignore_region_created`/`pick_candidates`로 돌려준다 — 후보를
실제로 pick할지 하나씩 물어보는 것만 호출자의 몫이다.

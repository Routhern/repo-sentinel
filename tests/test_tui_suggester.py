...
def test_suggests_command_prefix() -> None:
    assert compute_suggestion("prot", [], lambda *_: []) == "protect"


def test_no_suggestion_for_exact_or_unknown_command() -> None:
    assert compute_suggestion("protect", [], lambda *_: []) is None
...
def test_suggests_repo_key_for_second_token() -> None:
    ...
    result = compute_suggestion("protect appl", repo_keys, lambda *_: [])

    assert result == "protect apple-seed-factory"


def test_suggests_repo_key_by_host_prefix_for_portable_keys() -> None:
    repo_keys = ["github.com__me__apple-seed-factory"]

    result = compute_suggestion("protect git", repo_keys, lambda *_: [])

    assert result == "protect github.com__me__apple-seed-factory"


def test_suggests_relative_path_for_protect_third_token() -> None:
    def path_candidates(repo_key: str, prefix: str) -> list[str]:
        assert repo_key == "apple-seed-factory"
        assert prefix == "src/in"
        return ["src/input/"]

    result = compute_suggestion("protect apple-seed-factory src/in", [], path_candidates)

    assert result == "protect apple-seed-factory src/input/"


def test_no_suggestion_when_no_path_candidates() -> None:
    result = compute_suggestion("protect apple-seed-factory src/zzz", [], lambda *_: [])

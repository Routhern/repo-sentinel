...
from repo_sentinel.core import scanner, subscriptions, sync as sync_core
...
def scan(root: Path = typer.Argument(Path.home(), help="탐색을 시작할 루트 디렉터리")) -> None:
    ...
    console.print(table)
    console.print(f"[dim]추적하려면 `repo-sentinel subscribe <경로>`를 실행하세요.[/dim]")


@app.command()
def subscribe(path: Path = typer.Argument(..., help="추적할 git 저장소 경로")) -> None:
    """PATH를 구독 목록에 등록한다. 이때부터 protect/relink/audit 대상이 된다."""
    ...
    console.print(f"[green]구독 완료[/green]: {sub.repo_key} -> {sub.path}")


@app.command()
def unsubscribe(
    repo_key: str = typer.Argument(..., help="구독 해제할 repo_key"),
    mode: str = typer.Option(
        "restore",
        help="restore: vault 파일을 레포로 복원 후 해제(권장) / "
        "keep: 구독만 해제하고 vault 데이터는 보존 / "
        "purge: vault 데이터까지 즉시 삭제(레포에 깨진 링크가 남을 수 있음)",
    ),
) -> None:
    """구독을 해제한다. 보호 중이던 파일 처리 방식은 --mode로 선택한다."""
    ...
    sub = subs.get(repo_key)
    if sub is None:
        console.print(f"[red]{repo_key} 구독을 찾을 수 없습니다.[/red]")
        raise typer.Exit(code=1)
    ...
    # mode == "keep": vault 데이터는 그대로 두고 구독만 해제한다.

    subscriptions.remove_subscription(repo_key)
    console.print(f"[green]{repo_key} 구독 해제 완료[/green] (mode={mode})")
...
def list_repos() -> None:
    ...
    subs = subscriptions.load_subscriptions()
    if not subs:
        console.print("구독 중인 저장소가 없습니다. 먼저 `repo-sentinel subscribe`를 실행하세요.")
        raise typer.Exit()

    table = Table(title="repo-sentinel subscriptions")
    table.add_column("repo_key")
    table.add_column("경로")
    table.add_column("원격 URL")
    for sub in subs.values():
        table.add_row(sub.repo_key, sub.path, sub.remote_url or "-")
    console.print(table)


@app.command()
def status() -> None:
    """구독 중인 각 저장소의 git 상태(dirty 여부)를 요약한다."""
    subs = subscriptions.load_subscriptions()
    if not subs:
        console.print("구독 중인 저장소가 없습니다. 먼저 `repo-sentinel subscribe`를 실행하세요.")
        raise typer.Exit()
...
def protect(
    ...
    sub = subs.get(repo_key)
    if sub is None:
        console.print(f"[red]{repo_key} 구독을 찾을 수 없습니다. 먼저 subscribe하세요.[/red]")
        raise typer.Exit(code=1)
...
@app.command()
def relink(repo_key: Optional[str] = typer.Argument(None, help="생략 시 구독 전체를 재연결")) -> None:
    """매니페스트 기준으로 현재 머신의 심볼릭 링크를 재생성한다."""
    subs = subscriptions.load_subscriptions()
    config = load_config()
    vault_root = Path(config.vault_root)

    targets = [subs[repo_key]] if repo_key else list(subs.values())
    if repo_key and repo_key not in subs:
        console.print(f"[red]{repo_key} 구독을 찾을 수 없습니다.[/red]")
        raise typer.Exit(code=1)
    ...
    for sub in targets:
        try:
            ...
            console.print(f"[yellow]{sub.repo_key}: {e}[/yellow]")
            continue
        if relinked:
            console.print(f"[green]{sub.repo_key}[/green]: {', '.join(relinked)} 재연결")
        else:
            console.print(f"{sub.repo_key}: 변경 없음")


@app.command()
def audit(repo_key: Optional[str] = typer.Argument(None, help="생략 시 구독 전체를 점검")) -> None:
    ...
    all_issues = []
    for sub in targets:
        all_issues.extend(audit_core.audit_repo(Path(sub.path), sub.repo_key, vault_root))
